    ...
    return (
        <div className={clsx("rounded-2xl border p-5 flex flex-col h-full relative transition-all hover:shadow-md", s.bg, s.border)}>
            ...
            <div className="mt-auto pt-3 border-t border-black/5">
                <p className="text-xs font-bold text-gray-600 mb-1">Diagnóstico:</p>
    ...
    const sections = [
        {
            ...
            cards: [
                { title: 'Liquidez Corrente', value: formatNumber(ratios.liquidity.currentRatio, false), benchmark: '> 1.5', status: getStatus(ratios.liquidity.currentRatio, 1.5, 'higher'), diagnosis: diagnose(ratios.liquidity.currentRatio, 1.5, 'higher', 'Sua empresa tem folga financeira para pagar todas as obrigacoes de curto prazo.', 'Risco de liquidez. Voce pode ter dificuldade para pagar contas se houver um imprevisto.'), explanation: 'Para cada R$1,00 de divida vencendo no curto prazo, quanto você tem de bens (Caixa + Estoque + A Receber) para pagar.' },
                { title: 'Liquidez Seca', value: formatNumber(ratios.liquidity.quickRatio, false), benchmark: '> 1.0', status: getStatus(ratios.liquidity.quickRatio, 1.0, 'higher'), diagnosis: diagnose(ratios.liquidity.quickRatio, 1.0, 'higher', 'Você consegue pagar suas dividas sem depender de vender o estoque.', 'Você depende da venda do estoque para honrar compromissos.'), explanation: 'Igual a Liquidez Corrente, mas exclui o Estoque. Mostra a capacidade real de pagamento em um cenário de vendas fracas.' },
        ...
        },
        {
            ...
            cards: [
                { title: 'Divida Liquida / EBITDA', value: `${formatNumber(ratios.solvency.debtToEbitda, false)}x`, benchmark: '< 2.5x', status: getStatus(ratios.solvency.debtToEbitda, 2.5, 'lower'), diagnosis: diagnose(ratios.solvency.debtToEbitda, 2.5, 'lower', 'Nivel de endividamento saudavel e compativel com sua geração de caixa.', 'Endividamento perigoso. Sua operação demora muito para pagar o que deve aos bancos.'), explanation: 'Indica quantos anos de geração de caixa operacional (EBITDA) seriam necessarios para quitar toda a divida bancaria.' },
        ...
        },
        {
            ...
            cards: [
                ...
                { title: 'Retorno sobre Capital Investido (ROIC)', value: formatPercentage(ratios.profitability.roic), benchmark: '> Custo da Divida', status: getStatus(ratios.profitability.roic, 10, 'higher'), diagnosis: diagnose(ratios.profitability.roic, 10, 'higher', 'A operação e eficiente e gera valor acima do custo do dinheiro.', 'Destruicao de valor. A operação rende menos do que custa pegar dinheiro emprestado.'), explanation: 'Mede a eficiencia pura da operação em gerar retorno sobre todo o dinheiro investido (socios + bancos).' },
                { title: 'Payback do Custo de Aquisição (CAC)', value: `${formatNumber(ratios.efficiency.cacPayback, false)} meses`, benchmark: '< 12 meses', status: getStatus(ratios.efficiency.cacPayback, 12, 'lower'), diagnosis: diagnose(ratios.efficiency.cacPayback, 12, 'lower', 'Seu marketing se paga rapidamente. Voce pode acelerar o crescimento.', 'Você demora muito para recuperar o investimento em marketing. Risco de queimar caixa crescendo.'), explanation: 'Tempo (em meses) necessario para recuperar o custo de aquisição de um cliente atraves do lucro que ele gera.' },
            ]
        }
    ];
